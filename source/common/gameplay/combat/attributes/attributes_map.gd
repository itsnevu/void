...
const STRENGHT: Dictionary[StringName, float] = {
	...
	# damage growth. 60 pts ~ +36 AD, so a maxed STR build hits ~3-4x a fresh
...
const INTELLIGENCE: Dictionary[StringName, float] = {
	...
	# base AP (wand +18), INT scales it - 60 pts ~ +36 AP (x3 a fresh caster).
	# Also scales heal bolts (heal = AP x ratio), so INT is the support stat too.
	Stat.AP: 0.6,
}
...
const SPIRIT: Dictionary[StringName, float] = {
	...
	Stat.MANA_MAX: 1.0,
	# 60 pts ~ +1.2/s on the 0.5 base - a dedicated Spirit build refills ~3x faster.
	Stat.MANA_REGEN: 0.1,
}
